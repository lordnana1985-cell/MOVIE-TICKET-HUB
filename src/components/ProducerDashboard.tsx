...
export default function ProducerDashboard({
  ...
  const handleDeleteTicket = async (id: string) => {
    ...
    try {
      ...
      setSuccess('Movie premier ticket deleted successfully!');
      setTicketToDelete(null);
      await loadProducerData();
      onTicketCreated(); // notify parent
    } catch (err: any) {
      console.error(err);
      setError(err?.message || 'Failed to delete movie ticket.');
    } finally {
      setIsDeleting(null);
    }
  };
  ...
  const handleCreateTicket = async (e: FormEvent) => {
    ...
    try {
      ...
      if (coverSource === 'file' && coverFile) {
        setUploadStatus('Uploading cover artwork to Supabase "producers-assets" bucket...');
        const ext = coverFile.name.split('.').pop() || 'jpg';
        const uuid = `${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
        const cleanPath = `${user.id}/covers/${ticketId}/${uuid}.${ext}`;
        finalCoverUrl = await db.uploadFile('producers-assets', cleanPath, coverFile);
      }

      let formattedTrailer = trailerUrl.trim();
      if (videoSource === 'file' && videoFile) {
        setUploadStatus('Uploading trailer video to Supabase "producers-assets" bucket...');
        const ext = videoFile.name.split('.').pop() || 'mp4';
        const uuid = `${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
        const cleanPath = `${user.id}/videos/${ticketId}/${uuid}.${ext}`;
        formattedTrailer = await db.uploadFile('producers-assets', cleanPath, videoFile);
      } else {
      ...
      setSuccess('Movie Premier Ticket Generated successfully!');
      ...
      setError(err.message || 'Failed to generate ticket. Try again.');
    } finally {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <span className="text-xs font-mono tracking-widest text-gold font-semibold uppercase">
            {user.companyName || 'Cinema Production'} Studio Portal
          </span>
          <h2 className="font-display text-3xl font-extrabold tracking-tight text-white mt-1">
            Producer <span className="bg-gradient-to-r from-sky-light to-sky-deep bg-clip-text text-transparent">Console</span>
          </h2>
          <p className="text-sm text-gray-400 mt-1">
            Manage your movie premieres, upload trailers, generate tickets, and view real-time earnings.
          </p>
          {user.phoneNumber && (
        ...
        <div className="rounded-2xl glass-panel p-4 md:p-6 border-l-4 border-l-white/20 shadow-md">
          ...
          <div className="mt-4">
            ...
            <p className="text-[10px] text-gray-500 mt-1">
              Sent to Movie Ticket Hub platform
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleCreateTicket} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-gray-300 mb-1.5 font-mono">MOVIE TITLE *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-300 mb-1.5 font-mono">PREMIERE DESCRIPTION *</label>
                  <textarea
                    required
                    rows={4}
                    placeholder="Provide a compelling synopsis for movie goers and buyers..."
                    value={description}
            ...
            <div className="rounded-xl bg-white/5 border border-white/10 p-4 flex items-start gap-3">
              ...
              <div className="text-xs">
                ...
                Upon successful ticket purchases through Paystack, you receive <strong className="text-gold">80% of earnings</strong> directly into your account balance, while <strong className="text-sky-light">20% commission</strong> is routed back to Movie Ticket Hub.
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-lg font-bold text-white flex items-center gap-2">
              ...
              Your Generated Movie Premieres
            </h3>
              ...
              <form onSubmit={handleCreateSubaccount} className="space-y-3 animate-fadeIn">
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-white focus:border-gold outline-none font-medium"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-white focus:border-gold outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-white font-mono focus:border-gold outline-none"
                    />
                ...
                <div className="pt-2 flex gap-2">
                  {bankSubaccount && (
                    <button
                      type="button"
                      onClick={() => setIsEditingSubaccount(false)}
                  ...
                  <button
                    type="submit"
                    disabled={isSubmittingSubaccount || isLoadingBanks}
                  ...
                  >
                    {isSubmittingSubaccount ? (
                      ...
                      <span>Register & Link Account</span>
                    )}
                  </button>
                </div>
              </form>
